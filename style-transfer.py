...
def net(x):
	...
	for i, nc in enumerate(layer_sizes):
		out = layers.conv2d(out, nc, kernel_size=kernels[i], activation_fn=tf.nn.leaky_relu, trainable=False, reuse=tf.AUTO_REUSE, scope='conv%d'%i)
...
def merge(image, content, style, loss_at_level, weights=0.0):
	...
	with tf.name_scope('merge'):
		...
		if loss_at_level==-1:
			...
			for i in range(n):
				content_loss += wts[i]*tf.reduce_sum((outc[i]- outx[i])**2)*0.5
				g = tf.cast(tf.shape(outs[i]), 'float32')
				style_loss += wts[i]*tf.reduce_sum((getgram(outs[i])- getgram(outx[i]))**2)*0.25/g[1]/g[1]
		else:
			content_loss = tf.reduce_sum(outc[loss_at_level]- outx[loss_at_level])**2*0.5
			g = tf.cast(tf.shape(outs[loss_at_level]), 'float32')
			style_loss = tf.reduce_sum((getgram(outs[loss_at_level])- getgram(outx[loss_at_level]))**2)*0.25/g[1]/g[1]
...
def style_transfer(c,s):
	...
	op_train, op_loss, op_content, op_style = train(x, content, style, a=0.5, loss_at_level=-1)
	...
	with tf.Session() as sess:
		...
		with tf.name_scope('visualize'):
			outsc, outss = sess.run([op_c, op_s], feed_dict={content:c, style:s})
		...
		with tf.name_scope('optimize'):
			...
			for ii in range(20001):
				_, loss, closs, sloss = sess.run([op_train,op_loss,op_content,op_style], feed_dict={content:c, style:s})
				if ii%1000 == 0 or loss<1e-6: 
					print("loss @ %d = %f, c = %f, s =%f"%(ii,loss, closs, sloss))
					out = sess.run(x)
...
def main():
	content = sc.imresize(sc.imread('tubingen.jpg'),(img_size,img_size))/255.0
	style = sc.imresize(sc.imread('starry_night.jpg'), (img_size,img_size))/255.0
